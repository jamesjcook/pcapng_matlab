...
function sh=block_sh(in,  magic_numbers,  blk_len,  block_type)
...
%}
  ...
  sh.byte_magic=fread(in,1,'uint32=>uint32');
  ...
  sh.vers_major=fread(in,1,'uint16=>uint16');
  sh.vers_minor=fread(in,1,'uint16=>uint16');
  sh.section_length=fread(in,1,'int64=>int64');
  ...
  if opt_len>0
    % sh.options=fread(in,opt_len,'uint8=>uint8');
    sh.options=pcapng_option_read(in,opt_len,block_type);
  end
